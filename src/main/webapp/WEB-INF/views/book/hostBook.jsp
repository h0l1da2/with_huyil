...
<head>
  ...
  <style>
    ...
    .form-group.book-form.cancelBtn {
      width: 80px;
      position: relative;
      left: 10px;
    }
...
<div class="row block-9 info-menu">
      ...
      <form action="#" class="bg-white p-5 contact-form contact-book-form">
        <div class="delete-form">
          <div class="form-group book-form">
            <input type="text" id="bookId" value="${bookList.id}" hidden>
    ...
    <div class="row mt-5">
      <div class="col text-center">
        <div class="block-27">
          <ul>
            <c:if test="${ph.showPrev}"><li><a href="<c:url value="/hosts/info/book${ph.getPageInfo(ph.beginPage-1)}"/>">&lt;</a></li></c:if>
            <c:forEach var="i" begin="${ph.beginPage}" end="${ph.endPage}">
              <li<c:if test="${ph.viewPage eq i}"> class="active"</c:if>>
                <a href="<c:url value="/hosts/info/book${ph.getPageInfo(i)}"/>">${i}</a></li>
            </c:forEach>
            <c:if test="${ph.showNext}"><li><a href="<c:url value="/hosts/info/book${ph.getPageInfo(ph.endPage+1)}"/>">&gt;</a></li></c:if>
          </ul>
        </div>
      </div>
    </div>  </div>
